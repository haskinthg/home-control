...
STATE wait_for_level(gpio_num_t dht_pin, uint32_t level, uint32_t timeout) {
    uint32_t count = 0;
...
//      Ожидание от датчика низкий уровень сигнала
    STATE state = wait_for_level(dht_pin, LOW, 80);
    return state;
}
...
STATE read_dht(gpio_num_t dht_pin) {
    ...
    for(uint8_t i = 0; i < 5; i ++){
        for(uint8_t j = 0; j < 8; j++) {
            state = wait_for_level(dht_pin, HIGH, 50);
            if (state != SUCCESS) return state;
            ets_delay_us(30);

            if(gpio_get_level(dht_pin) == 1) data[i] |= (1 << (8-j));
            state = wait_for_level(dht_pin, LOW, 100);
            if (state != SUCCESS) return state;
        }
    }
